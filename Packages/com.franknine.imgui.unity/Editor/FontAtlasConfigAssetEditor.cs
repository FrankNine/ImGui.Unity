...
using ImGui.Unity.FreeType;

namespace ImGui.Unity.Editor
{
    ...
    internal class FontAtlasConfigAssetEditor : UnityEditor.Editor
    {
        ...
        {
            public static GUIContent rasterizerFlags = new("Rasterizer Flags",
                "Settings for custom font rasterizer. Forces flags on all fonts.");

            public static GUIContent fonts = new("Fonts", "Fonts to pack into the atlas texture.");
        }
        ...
        private void OnEnable()
        {
            ...
            _fontsList = new ReorderableList(serializedObject, _fonts, true, true, true, true)
            {
                elementHeightCallback = (index) =>
                    EditorGUI.GetPropertyHeight(_fontsList.serializedProperty.GetArrayElementAtIndex(index)) +
                ...
                drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                {
                    if (index % 2 != 0)
                    {
                        EditorGUI.DrawRect(new Rect(rect.x - 19f, rect.y, rect.width + 23f, rect.height),
                            new Color(0, 0, 0, .1f));
                    }
