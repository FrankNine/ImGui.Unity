    ...
    internal class FontDefinitionDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (string.IsNullOrEmpty(fontPath.stringValue))
            {
                ...
                EditorGUI.HelpBox(position, $"Font file must be in '{EditorStreamingAssetsPath}' folder.",
                    MessageType.Error);
            }
            else
            {
        ...
        private string GetStreamingAssetPath(SerializedProperty property)
        {
            string path = property.objectReferenceValue != null
                ...
                : string.Empty;
            return path.StartsWith(EditorStreamingAssetsPath)
                ? path.Substring(EditorStreamingAssetsPath.Length)
                : string.Empty;
        }
    }
}