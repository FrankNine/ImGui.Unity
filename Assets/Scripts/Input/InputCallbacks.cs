...
namespace ImGui.Unity.Input 
{
    ...
    internal unsafe class InputCallbacks
    {
        ...
        {
            GUIUtility.systemCopyBuffer = ImGuiUtilities.StringFromPtr(text);
        }
        ...
        {
            UnityEngine.Input.compositionCursorPos = new Vector2(x, y);
        }
        ...
        public static SetClipboardTextSafeCallback SetClipboardText
        {
            set => _setClipboardText = (user_data, text) =>
            {
                try
                {
                    value(new IntPtr(user_data), ImGuiUtilities.StringFromPtr(text));
                }
                catch (Exception ex)
                {
                    Debug.LogException(ex);
